...
test('genesi, giorno, vendita, tetto, interessi, estratto: righe accodate e verificate; il motore le rilegge', async () => {
  const d = dir();
  const ledger = join(d, 'ledger.jsonl');
  const { frase } = c.nuovaFrase();
  const g = await c.genesi(ledger, frase, { tetto_cent: 400000 });
  assert.equal(g.riga.seq, 0);
  assert.equal(g.riga.type, 'genesis');
  await assert.rejects(async () => await c.genesi(ledger, frase), /la genesi c'è già/);
  await assert.rejects(async () => await c.giorno(ledger, generaFrase(), '2026-09-01'), /non è quella della banca/);
  await c.giorno(ledger, frase, '2026-09-01');
  const cliente = portafoglioDaFrase(generaFrase());
  const v = await c.vendita(ledger, frase, '1000', cliente.coordinate, 'bonifico 12');
  ...
  await c.tetto(ledger, frase, '5000');
  await c.interessi(ledger, frase, '2,50', '2026-08-01', '2026-08-31');
  await c.estratto(ledger, frase, '2026-08', '1002,50', 'd'.repeat(64));
  await assert.rejects(async () => await c.estratto(ledger, frase, '2026-09', '999', 'd'.repeat(64)), /serve una nota/);
  const s = await c.stato(ledger);
...
test('conversioni: la banca legge i dati con la sua frase, esegue al prezzo del giorno, segna pagata', async () => {
  const d = dir();
  const ledger = join(d, 'ledger.jsonl');
  const { frase, coordinate: coordBanca } = c.nuovaFrase();
  await c.genesi(ledger, frase);
  await c.giorno(ledger, frase, '2026-09-01');
  const cliente = portafoglioDaFrase(generaFrase());
  await c.vendita(ledger, frase, '500', cliente.coordinate, 'b');
  ...
  const lista = await c.conversioniInAttesa(ledger, frase);
  ...
  assert.equal((await c.stato(ledger)).in_attesa, 1);
  const e = await c.esegui(ledger, frase, lista[0].hash);
  assert.equal(e.euro, 19600n);
  await c.pagata(ledger, frase, lista[0].hash, '2026-09-02');
  assert.equal((await c.conversioniInAttesa(ledger, frase)).length, 0);
  assert.equal((await c.stato(ledger)).riserva_cent, 50000n - 19600n);
  await assert.rejects(async () => await c.esegui(ledger, frase, 'f'.repeat(64)), /sconosciuta/);
});
