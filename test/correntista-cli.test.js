...
test('conto di prova: nuovo, coordinate, vendita dalla banca, pagamento riservato, saldo, conversione', () => {
  const d = mkdtempSync(join(tmpdir(), 'manti-cc-'));
  const ledger = join(d, 'ledger.jsonl');
  const { frase: fraseBanca } = banca.nuovaFrase();
  banca.genesi(ledger, fraseBanca);
  banca.giorno(ledger, fraseBanca, '2026-09-01');
  const anna = cc.nuovo();
  ...
  assert.deepEqual(cc.saldo(ledger, anna.frase), { totale: 0, valore: null, in_euro: null, entrate: [] });

  banca.vendita(ledger, fraseBanca, '50', anna.coordinate, 'bonifico 1');
  let s = cc.saldo(ledger, anna.frase);
  assert.equal(s.totale, 5000);
  assert.equal(s.in_euro, 5000n);
  assert.equal(s.entrate[0].tag, 'vendita');

  const p = cc.paga(ledger, anna.frase, '12,50', bruno.coordinate, 'pizza');
  assert.equal(p.riga.type, 'transfer');
  assert.equal(p.resto, 3750);
  s = cc.saldo(ledger, bruno.frase);
  ...
  assert.throws(() => cc.converti(ledger, bruno.frase, '1', ''), /dati per il pagamento/);
  const attesa = banca.conversioniInAttesa(ledger, fraseBanca);
  assert.equal(attesa.length, 1);
  assert.equal(attesa[0].dati, 'Bruno Neri, IT11A0000000000000000000000');
  banca.esegui(ledger, fraseBanca, attesa[0].hash);
  assert.equal(cc.saldo(ledger, bruno.frase).totale, 250);
